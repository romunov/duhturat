createFigures <- function(xy, type = c("N", "E"), ...) {
  ...
  if (names(ps) %in% "a1") {
    ...
    ggsave(sprintf("./figures/%s-1a.gostota gled na razmerje hr_sap po correction type in st. gen.walk.png", type),
  ...
  if (names(ps) %in% "b1") {
    ggplot(xe, aes(x = hr.sap.ratio, y = index)) +
      ...
      geom_hline(yintercept = 1, size = 0.5, alpha = 0.5) +
      facet_grid(num.generated.walkers ~ correction.type)
    ggsave(sprintf("./figures/%s-1b.gostota gled na razmerje hr_sap po correction type in st. gen.walk brez pik.png", type),
           width = fw, height = fh, units = "in")
  }
  ...
  if (names(ps) %in% "a2") {
    ggplot(xe, aes(x = hr.sap.ratio, y = index)) +
      ...
      geom_hline(yintercept = 1, size = 0.5, alpha = 0.5) +
      facet_grid(num.generated.walkers ~ correction.type)
    ggsave(sprintf("./figures/%s-2a.gostota glede na razmerje, corr.type in st. gen walkerjev pozoomano.png", type),
  ...
  if (names(ps) %in% "a5") {
    ...
    ggplot(droplevels(xe[xe$model %in% c(".1", ".sp"), ]), aes(x = hr.sap.ratio, y = dAIC)) +
      ...
      scale_color_brewer(palette = "Set1") +
      facet_grid(num.generated.walkers ~ correction.type)
    ggsave(sprintf("./figures/%s-5a.dAIC gled na razmerje hr_sap po correction type in st. gen.walk.png", type),
           width = 10, height = 8, units = "in")
  }
