...
ane <- sapply(ae, FUN = function(x) {
  ...
  qs <- getQcustom(fun = weibullLikeDistribution, sigma = sigma, b = b, mx = mx, xrange = c(0, sigma * 3))
...
xy$correction.type <- sapply(strsplit(xy$variable, "\\."), "[", 2)
