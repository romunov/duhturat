...
#	$ effect.distance: num 26.7
  ...
  # požene čez funkcijo v runme.R. Ideja za ime: simulateSP
    
    
    
  # KO BO IMPLEMENTIRAN ZGORNJI TODO, BO TALE SPODNJI CHUNK OBSOLETE  
...
# prispevek v območje vzorčenja glede na dejanski centroid walkerja
  ...
  cxy <- apply(xy, 1, function(m) {
      data.frame(x = m[1], y = m[2], capt = m[3])
    })
  names(cxy) <- 1:length(cxy)
  
  moco <- individualContribution(walk = cxy, ...object = ..object, 
    .sap.poly = sap.poly, effect.distance = my.bins$effect.distance,
    ring.weights = my.bins$bins)
